...
IndexGeometry.prototype.orientedFaceToTri = function( geometry, face ){
	...
	// TODO: We need 2 faces to compute the angle
	var angle = 0.0;
...
function buildTriFold( geometry, tree, root, angle ){
	...
	//console.info( tree );
	if( tree.right ){
		var parms = IndexGeometry.prototype.orientedFaceToTri( geometry, tree.right.face );
		root.right.extend( parms.position, parms.len, angle );
		buildTriFold( geometry, tree.right, root.right, angle );
	}
	if( tree.left ){
		var parms = IndexGeometry.prototype.orientedFaceToTri( geometry, tree.left.face );
		root.left.extend( parms.position, parms.len, angle );
		buildTriFold( geometry, tree.left, root.left, angle );
	}
	if( tree.base ){
		var parms = IndexGeometry.prototype.orientedFaceToTri( geometry, tree.base.face );
		root.base.extend( parms.position, parms.len, angle );
